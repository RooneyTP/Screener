...
FILES_TO_RUN = [
    os.path.join(BASE_DIR, "1_producer_data.py"),
    os.path.join(BASE_DIR, "2_consumer_ai.py"),
    os.path.join(BASE_DIR, "3_consumer_r1.py"),
]
...
def clean_database():
    ...
    if os.path.exists(DB_NAME):
        ...
        if file_time.date() < hari_ini:
            try:
                os.remove(DB_NAME)
                print(f"\U0001f9f9 [CLEANUP] Database '{DB_NAME}' sisa kemarin berhasil dihapus.")
...
def main():
    print("==================================================")
    print("  \U0001f916 SUPER MANAGER: BOT SCALPING OTOMATIS AKTIF  ")
    print("==================================================")
    ...
    try:
        while True:
            if is_market_open():
                ...
                if not processes:
                    print(f"\n\U0001f7e2 [START] Jam bursa aktif! Menyalakan {len(FILES_TO_RUN)} mesin tempur...")
                    for file in FILES_TO_RUN:
                        if os.path.exists(file):
                            # \U0001f7e2 UPGRADE: Tambahkan "-u" (Unbuffered) agar log terminal muncul real-time!
                ...
                if processes:
                    print("\n\U0001f534 [STOP] Jam bursa telah usai. Menarik mundur semua mesin...")
                    for p in processes:
                        p.terminate() # Matikan paksa subprocess
                    processes = []
                    print("\U0001f4a4 [STATUS] Mesin sedang beristirahat.")
        ...
        print("\n\n\U0001f6d1 [SHUTDOWN] Mematikan Super Manager secara manual...")
        for p in processes:
            p.terminate()
        print("\U0001f44b Sampai jumpa, Bos!")

# FIX: APScheduler placeholder for auto-training (SKILL.md recommends)
# To enable: pip install apscheduler, then uncomment below
#
# from apscheduler.schedulers.background import BackgroundScheduler
# def scheduled_training():
#     """Run AI training daily after market close."""
#     import subprocess, sys
#     subprocess.run([sys.executable, "latih_ai.py"])
#     subprocess.run([sys.executable, "auto_train.py"])
#     print("[SCHEDULER] AI training completed.")
#
# def start_scheduler():
#     scheduler = BackgroundScheduler()
#     scheduler.add_job(scheduled_training, 'cron', day_of_week='mon-fri', hour=16, minute=30)
#     scheduler.start()
#     return scheduler
