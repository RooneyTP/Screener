...
# ── Signal manager (cooldown only — sector cap butuh data sektor per ticker) ──
...
# ── Market regime — compute ONCE from IHSG, not per ticker ──
...
logger.info("Market regime: %s (ADX=%.1f, trend=%.1f)", regime, adx_val, trend_score)
...
# ── Market sentiment (prediksi besok) — dihitung sekali, dipakai untuk entry_rec ──
...
for tkr in WATCHLIST:
    try:
        ...
        # Entry recommendation — NOW with market_sentiment context
        ...
        # Record cooldown — regardless of swing or intra signal
