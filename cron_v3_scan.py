#!/usr/bin/env python3
"""Cron Wrapper — IDX Alpha Screener v5 (21:00 WIB)"""
...
print(f"{'='*60}\n  IDX ALPHA SCREENER v5 — {now}\n  Engine: 3 Profile + Dynamic Percentile\n{'='*60}")
sys.stdout.flush()
try:
    result = subprocess.run(
        [sys.executable, "main.py", "--top", "125", "--no-ihsg", "--telegram", "--v6"],
...
if os.path.exists(csv_src):
    ...
    with open(os.path.join(OUTPUT_DIR, f"screener_v5_{today}.csv"), "w") as f: f.write(data)
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "screener_v5_result.csv"), "w") as f: f.write(data)
    print(f"CSV saved")
